...
def compare_with_baseline(gnn_model, logreg_model, val_dataset):
    ...
    with torch.no_grad():
        for data in val_dataset:
            ...
            x, y, edge_attr = data.x.cpu().numpy(), data.y.cpu().numpy(), data.edge_attr.cpu().numpy()
            graph_node_features = np.mean(x, axis=0)
            if edge_attr is not None:
                graph_edge_features = np.mean(edge_attr, axis=0)
...
def main():
    ...
    GINE_model = train_GINE(train_dataset, num_epochs=200, lr=0.001, dropout_prob=0.15, batch_size=16)
